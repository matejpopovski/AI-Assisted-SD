...
Read that module's `CLAUDE.md` (or `README.md`, if your team built the context hierarchy that
way) plus its ancestors up to the root of the context hierarchy — not sibling modules unless
the feature genuinely spans them, and not raw source beyond what's needed to confirm a summary
is still accurate.
