...
`host-roster-management.md`), kept forever. Never a single shared file that the next feature
overwrites — a design doc is only useful as history if later work doesn't erase it.

This project's instructions (T3/T4) call this document `docs/design-<feature>.md` — same idea,
different filename convention. Either location is graded the same way; use whichever this file
path suggests, or your team's own convention, as long as it's one durable file per feature under
`docs/`.
